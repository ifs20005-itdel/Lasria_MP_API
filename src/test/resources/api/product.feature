Feature: product
  ...
  Scenario Outline: Send GET http request to get product data by id
    Given I set GET endpoints by id
    When I send GET HTTP request by id
    Then I receive a valid HTTP response code <ignoredArg0> for Get id data products
  ...
  Scenario Outline: POST As admin I have be able to create new products rating
    Given I set POST endpoints by rating
    When I send POST HTTP request by rating
    Then I receive valid HTTP response code <ignoredArg0> in products rating POST
    And I receive valid data for assign product rating
    Examples:
      | ignoredArg0 |
      | 200         |

  Scenario Outline: POST As admin I have be able to create new products comment
    Given I set POST endpoints by comment
    When I send POST HTTP request by comment
    Then I receive valid HTTP response code <ignoredArg0> in products comment POST
